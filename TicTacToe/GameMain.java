...
/* This class contains the main method and handles the gameplay. The class
		GameMain controls the logical flow of the game, which will handle moving between
		the player turns and updating the game state. It declares the constants that specify
		how many cells are on the board and the dimensions for most of the drawn elements
		in the game (canvas, cell, symbol size and stroke width). The constructor sets up the
		Mouse Listener to listen for the Mouse Click events, sets up the status label that will
		show who’s turn it is and whether a player has won or drawn, creates a new Board
		object and calls the method to initialise the game board content. The GameMain
		class supports operations of the overall gameplay with UpdateGame(). */
